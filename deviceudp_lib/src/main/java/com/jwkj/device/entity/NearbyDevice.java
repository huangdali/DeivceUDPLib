...
public class NearbyDevice {
    ...
    public static NearbyDevice getDeviceInfoByByteArray(byte[] data) {
        ...
        int msgVersion = MUtils.getInt(data,12);
        int curVersion = (msgVersion >> 4) & 0x1;
        device.setDeviceType(MUtils.getInt(data,20));
        device.setDeviceSubType(MUtils.getInt(data,80));
        device.setDeviceId(MUtils.getInt(data,16));
        device.setCmd(MUtils.getInt(data,0));
        device.setPwdFlag(MUtils.getInt(data,24));
